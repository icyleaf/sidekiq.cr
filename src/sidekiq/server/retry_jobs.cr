    ...
    class RetryJobs < ServerEntry
      ...
      def attempt_retry(job, ctx, exception)
        ...
        count = if job.retry_count.nil?
                  job.failed_at = Time.now(Sidekiq.default_timezone)
                  job.retry_count = 0
                else
                  job.retried_at = Time.now(Sidekiq.default_timezone)
        ...
        if count < max_retry_attempts
          ...
          retry_at = Time.now(Sidekiq.default_timezone) + delay.seconds
          payload = job.to_json
      ...
      def send_to_morgue(job, ctx)
        ...
        payload = job.to_json
        now = Time.now(Sidekiq.default_timezone)
        ctx.pool.redis do |conn|
          conn.multi do

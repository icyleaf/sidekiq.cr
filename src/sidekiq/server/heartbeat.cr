  ...
  class Heartbeat
    ...
    def server_json(svr)
      data = {
        ...
        "started_at"  => Time.now(Sidekiq.default_timezone).to_unix_f,
        "pid"         => ::Process.pid,
        "tag"         => svr.tag,
    ...
    def ❤(svr, json)
      ...
      begin
        ...
        nowdate = @daily.format(Time.now(Sidekiq.default_timezone))
        svr.pool.redis do |conn|
          conn.multi do |multi|
        ...
        _, _, _, msg = svr.pool.redis do |conn|
          conn.multi do |multi|
            ...
            multi.hmset(svr.identity, {"info"  => json,
                                       ...
                                       "beat"  => Time.now(Sidekiq.default_timezone).to_unix_f,
                                       "quiet" => svr.stopping?})
